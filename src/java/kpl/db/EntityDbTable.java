...
public abstract class EntityDbTable<T> extends DerivedDbTable {
    ...
    public void checkAvailable(int height) {
        if (multiversion) {
            int minRollBackHeight = isPersistent() && Kpl.getBlockchainProcessor().isScanning() ?
                    Math.max(Kpl.getBlockchainProcessor().getInitialScanHeight() - Constants.MAX_ROLLBACK, 0)
    ...
    private boolean doesNotExceed(int height) {
        return Kpl.getBlockchain().getHeight() <= height && ! (isPersistent() && Kpl.getBlockchainProcessor().isScanning());
    }
