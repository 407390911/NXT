...
import kpl.http.JSONResponses;
import kpl.http.ParameterParser;
...
public final class JPLSnapshot implements AddOn {
     ...
     * requirement to existing KPL holders.</p>
     *
     * <p>This utility takes a snapshot of account balances and public keys on the Kpl blockchain as of the specified height,
     ...
     * <li>height - the Kpl blockchain height at which to take the snapshot</li>
     * </ul>
     *
     * <p>Response</p>
     ...
     * for a total of 900M to new accounts, resulting in 100M automatically allocated to existing KPL holders:
     * <pre>
     * {
     *     "balances": {
     *         "KPL-NZKH-MZRE-2CTT-98NPZ": 30000000000000000,
     *         "KPL-X5JH-TJKJ-DVGC-5T2V8": 30000000000000000,
     *         "KPL-LTR8-GMHB-YG56-4NWSE": 30000000000000000
     *     },
     *     "publicKeys": [
    ...
    public static class JPLSnapshotAPI extends APIServlet.APIRequestHandler {
        ...
        protected JSONStreamAware processRequest(HttpServletRequest request, HttpServletResponse response) throws KplException {
            ...
            Kpl.getBlockchainProcessor().addListener(listener, BlockchainProcessor.Event.AFTER_BLOCK_ACCEPT);
            Kpl.getBlockchainProcessor().scan(height - 1, false);
            Kpl.getBlockchainProcessor().removeListener(listener, BlockchainProcessor.Event.AFTER_BLOCK_ACCEPT);
    ...
    private static class JPLSnapshotListener implements Listener<Block> {
        ...
        public void notify(Block block) {
            if (block.getHeight() == height) {
                ...
                SortedMap<String, Long> snapshotKplBalances = snapshotKplBalances();
                BigInteger snapshotTotal = BigInteger.valueOf(snapshotKplBalances.values().stream().mapToLong(Long::longValue).sum());
                ...
                if (inputBalances != null) {
                    ...
                    if (!inputTotal.equals(BigInteger.ZERO)) {
                        snapshotKplBalances.entrySet().forEach(entry -> {
                    ...
                    inputBalances.entrySet().forEach(entry -> {
                        ...
                        snapshotKplBalances.merge(account, inputBalance, (a, b) -> a + b);
                    });
                }
                snapshot.put("balances", snapshotKplBalances);
            }
        }
        ...
        private SortedMap<String, Long> snapshotKplBalances() {
