...
var KRS = (function (KRS, $, undefined) {
    ...
    KRS.getPhasingDetails = function(phasingDetails, phasingParams) {
        ...
        if (phasingParams.phasingWhitelist && phasingParams.phasingWhitelist.length > 0) {
            ...
            for (var i = 0; i < phasingParams.phasingWhitelist.length; i++) {
                ...
                rows += "<tr><td><a href='#' data-user='" + KRS.escapeRespStr(account) + "' class='show_account_modal_action'>" + KRS.getAccountTitle(account) + "</a></td></tr>";
            }
            rows += "</tbody></table>";
        } else {
            rows = "-";
    ...
    KRS.processTransactionModalData = function (transaction, isModalVisible, sharedKey) {
        ...
        try {
            ...
            $("#transaction_info_modal_transaction").html(KRS.escapeRespStr(transaction.transaction));
            ...
            if (transaction.type == 0) {
                    ...
                    case 0:
                        ...
                        if (transaction.attachment) {
                            if (transaction.attachment.message) {
                                ...
                                $output.html("<div style='color:#999999;padding-bottom:10px'><i class='fa fa-unlock'></i> " + $.t("public_message") + "</div><div style='padding-bottom:10px'>" + KRS.escapeRespStr(message).nl2br() + "</div>");
                            }
                            ...
                            KRS.sendRequest("getAlias", {
                                ...
                                if (!response.errorCode) {
                                    if (transaction.recipient != response.buyer || transaction.attachment.priceNQT != response.priceNQT) {
                                        ...
                                        }) + " <a href='#' data-alias='" + KRS.escapeRespStr(transaction.attachment.alias) + "' data-toggle='modal' data-target='#buy_alias_modal'>" + $.t("buy_it_q") + "</a>";
                                        ...
                                        if (transaction.attachment.priceNQT != "0") {
                                            message = $.t("your_alias_sale_offer") + " <a href='#' data-alias='" + KRS.escapeRespStr(transaction.attachment.alias) + "' data-toggle='modal' data-target='#cancel_alias_sale_modal'>" + $.t("cancel_sale_q") + "</a>";
                                        }
                                    } else {
                ...
                switch (transaction.subtype) {
                    case 0:
                        KRS.sendRequest("getAsset", {
                            ...
                            data["sender"] = transaction.senderRS ? transaction.senderRS : transaction.sender;
                            $("#transaction_info_callout").html("<a href='#' data-goto-asset='" + KRS.escapeRespStr(transaction.transaction) + "'>Click here</a> to view this asset in the Asset Exchange.").show();
                            ...
                            KRS.sendRequest("getDGSPurchase", {
                                ...
                                if (purchase.errorCode) {
                                    if (purchase.errorCode == 4) {
                                        ...
                                        callout = KRS.escapeRespStr(purchase.errorDescription);
                                    }
                                } else {
                                    if (KRS.account == transaction.recipient || KRS.account == transaction.sender) {
                                        if (purchase.pending) {
                                            if (KRS.account == transaction.recipient) {
                                                callout = "<a href='#' data-toggle='modal' data-target='#dgs_delivery_modal' data-purchase='" + KRS.escapeRespStr(transaction.transaction) + "'>" + $.t("deliver_goods_q") + "</a>";
                                            } else {
                            ...
                            KRS.sendRequest("getDGSGood", {
                                ...
                                if (KRS.account == purchase.buyer) {
                                    if (purchase.refundNQT) {
                                        ...
                                        callout = $.t("goods_received") + " <a href='#' data-toggle='modal' data-target='#dgs_feedback_modal' data-purchase='" + KRS.escapeRespStr(transaction.attachment.purchase) + "'>" + $.t("give_feedback_q") + "</a>";
                                    }
                                ...
                                if (purchase.seller == KRS.account || purchase.buyer == KRS.account) {
                                    KRS.sendRequest("getDGSPurchase", {
                                        ...
                                        if (purchase.buyer == KRS.account) {
                                            ...
                                            if (!purchase.refundNQT) {
                                                callout = "<a href='#' data-toggle='modal' data-target='#dgs_refund_modal' data-purchase='" + KRS.escapeRespStr(transaction.attachment.purchase) + "'>" + $.t("refund_this_purchase_q") + "</a>";
                                            } else {
                                                callout = $.t("purchase_refunded");
                                            }
                                        }
                ...
                switch (transaction.subtype) {
                    case 0:
                        ...
                        var currencyTypes = "";
                        for (var currencyType in KRS.constants.CURRENCY_TYPES) {
                            if (!KRS.constants.CURRENCY_TYPES.hasOwnProperty(currencyType)) {
                                continue;
                            }
                            //noinspection JSBitwiseOperatorUsage
                            if (transaction.attachment.type & KRS.constants.CURRENCY_TYPES[currencyType]) {
                                currencyTypes += currencyType + ",";
                            }
                        }
                        currencyTypes = transaction.attachment.type + ":" + currencyTypes.slice(0, -1);
                        var hashAlgorithm = transaction.attachment.algorithm;
                        for (var algorithm in KRS.constants.MINTING_HASH_ALGORITHMS) {
                            if (!KRS.constants.MINTING_HASH_ALGORITHMS.hasOwnProperty(algorithm)) {
                                continue;
                            }
                            if (transaction.attachment.algorithm == KRS.constants.MINTING_HASH_ALGORITHMS[algorithm]) {
                ...
                if (!incorrect) {
                    ...
                    if (currency != null && KRS.isExchangeable(currency.type)) {
                        infoCallout.append("<a href='#' data-goto-currency='" + KRS.escapeRespStr(currency.code) + "'>" + $.t('exchange_booth') + "</a><br/>");
                    }
                    if (currency != null && KRS.isReservable(currency.type)) {
                        infoCallout.append("<a href='#' data-toggle='modal' data-target='#currency_founders_modal' data-currency='" + KRS.escapeRespStr(currency.currency) + "' data-name='" + KRS.escapeRespStr(currency.name) + "' data-code='" + KRS.escapeRespStr(currency.code) + "' data-ressupply='" + KRS.escapeRespStr(currency.reserveSupply) + "' data-initialsupply='" + KRS.escapeRespStr(currency.initialSupply) + "' data-decimals='" + KRS.escapeRespStr(currency.decimals) + "' data-minreserve='" + KRS.escapeRespStr(currency.minReservePerUnitNQT) + "' data-issueheight='" + KRS.escapeRespStr(currency.issuanceHeight) + "'>View Founders</a><br/>");
                    }
                    if (currency != null) {
                        infoCallout.append("<a href='#' data-toggle='modal' data-target='#currency_distribution_modal' data-code='" + KRS.escapeRespStr(currency.code) + "'  data-i18n='Currency Distribution'>Currency Distribution</a>");
                    }
                    infoCallout.show();
            ...
            if (!(transaction.type == 1 && transaction.subtype == 0)) {
                if (transaction.attachment) {
                    ...
                    if (transaction.attachment.message) {
                        ...
                        transactionInfoOutputBottom.append("<div style='padding-left:5px;'><label><i class='fa fa-unlock'></i> " + $.t("public_message") + "</label><div>" + KRS.escapeRespStr(message).nl2br() + "</div></div>");
                    }
    ...
    KRS.formatCurrencyOffer = function (currency, transaction) {
        ...
        if (buyOffer && sellOffer) {
            data["buy_supply_formatted_html"] = KRS.formatQuantity(buyOffer.supply, currency.decimals) + " (initial: " + KRS.formatQuantity(transaction.attachment.initialBuySupply, currency.decimals) + ")";
            data["buy_limit_formatted_html"] = KRS.formatQuantity(buyOffer.limit, currency.decimals) + " (initial: " + KRS.formatQuantity(transaction.attachment.totalBuyLimit, currency.decimals) + ")";
            data["buy_rate_formatted_html"] = KRS.calculateOrderPricePerWholeQNT(transaction.attachment.buyRateNQT, currency.decimals) + rateUnitsStr;
            data["sell_supply_formatted_html"] = KRS.formatQuantity(sellOffer.supply, currency.decimals) + " (initial: " + KRS.formatQuantity(transaction.attachment.initialSellSupply, currency.decimals) + ")";
            data["sell_limit_formatted_html"] = KRS.formatQuantity(sellOffer.limit, currency.decimals) + " (initial: " + KRS.formatQuantity(transaction.attachment.totalSellLimit, currency.decimals) + ")";
            data["sell_rate_formatted_html"] = KRS.calculateOrderPricePerWholeQNT(transaction.attachment.sellRateNQT, currency.decimals) + rateUnitsStr;
        }
        var rows = "";
