...
var KRS = (function (KRS, $, undefined) {
    ...
    KRS.isExchangeable = function (type) {
        return type & KRS.constants.CURRENCY_TYPES["EXCHANGEABLE"];
    };

    KRS.isControllable = function (type) {
        return type & KRS.constants.CURRENCY_TYPES["CONTROLLABLE"];
    };

    KRS.isReservable = function (type) {
        return type & KRS.constants.CURRENCY_TYPES["RESERVABLE"];
    };

    KRS.isClaimable = function (type) {
        return type & KRS.constants.CURRENCY_TYPES["CLAIMABLE"];
    };

    KRS.isMintable = function (type) {
        return type & KRS.constants.CURRENCY_TYPES["MINTABLE"];
    };

    KRS.isNonShuffleable = function (type) {
        return type & KRS.constants.CURRENCY_TYPES["NON_SHUFFLEABLE"];
    };
    ...
    search.on("submit", function (e, data) {
        ...
        $(".currency_code").html(KRS.escapeRespStr(currencyCode));
    ...
    $("#currencies_search").on("submit", function (e) {
        ...
        KRS.sendRequest(requestType, params, function (response) {
    ...
    function getAllCurrenciesRows(response) {
        ...
        for (var i = 0; i < response.currencies.length; i++) {
            ...
            var minReserve = KRS.escapeRespStr(currency.minReservePerUnitNQT);
    ...
    function processOffers(offers, type, refresh, isIgnis) {
        ...
        if (offers && offers.length) {
            ...
            for (i = 0; i < offers.length; i++) {
                ...
                if (i == 0 && !refresh) {
                    rate.val(KRS.calculateOrderPricePerWholeQNT(rateNQT, decimals));
                    supply.text(KRS.formatQuantity(offer.supply, decimals));
                }
                rows += "<tr>" +
            ...
            if (isIgnis) {
                ...
                KRS.sendRequest("getAccountProperties", {
                    "recipient": ignisCurrencyIssuer.val(),
                    "setter": ignisCurrencyIssuer.val(),
                    "property": KRS.constants.IGNIS_CURRENCY_CODE + "rate"
                }, function(propertiesResponse) {
                    if (propertiesResponse.properties.length) {
                        rate.val(KRS.calculateOrderPricePerWholeQNT(propertiesResponse.properties[0].value, decimals));
                    }
                }, { isAsync: false });
            } else {
                rate.val("0");
    ...
    KRS.loadCurrencyOffers = function (type, currencyId, refresh, isIgnis) {
        async.parallel([
            function(callback) {
    ...
    KRS.getExchangeHistory = function (currencyId, refresh, tableName) {
        var historyTable;
        if (tableName) {
        ...
        if (KRS.currenciesTradeHistoryType == "my_exchanges" && !tableName) {
                ...
                if (response.exchanges && response.exchanges.length) {
                    ...
                    for (var i = 0; i < response.exchanges.length; i++) {
                        ...
                        rows += "<tr>" +
                            "<td>" + KRS.getTransactionLink(exchange.transaction, KRS.formatTimestamp(exchange.timestamp)) + "</td>";
                            if (!tableName) {
                                rows += "<td>" + KRS.getAccountLink(exchange, "seller") + "</td>";
                            }
                            rows += "<td>" + KRS.getAccountLink(exchange, "buyer") + "</td>" +
                            "<td class='numeric'>" + KRS.formatQuantity(exchange.units, exchange.decimals, false, quantityDecimals) + "</td>" +
                            "<td class='numeric'>" + KRS.formatOrderPricePerWholeQNT(exchange.rateNQT, exchange.decimals, rateNQTDecimals) + "</td>" +
                            "<td class='numeric'>" + KRS.formatAmount(KRS.calculateOrderTotalNQT(exchange.units, exchange.rateNQT), false, false, totalNQTDecimals) + "</td>" +
                        "</tr>";
    ...
    KRS.forms.orderCurrency = function () {
        ...
        if (orderType == "currencyBuy" && currencyCode != "JLRDA" && KRS.isShowFakeWarning()) {
            return KRS.composeFakeWarning($.t("currency"), currencyCode);
        }
    ...
    unitFields.on("change", function() {
        ...
        var units = KRS.convertToQNT(String($(units_field).val()), decimals);
        KRS.sendRequest("getAvailableTo" + KRS.initialCaps(orderType), {
            ...
            if (response.rateNQT == "0") {
                if (KRS.isDisableScheduleRequest()) {
                    submitButton.prop('disabled', true);
                ...
                KRS.sendRequest("getAccountProperties", {
                    ...
                    if (propertiesResponse.properties.length) {
                        var rate = KRS.calculateOrderPricePerWholeQNT(propertiesResponse.properties[0].value, decimals);
                        rateField.val(rate);
                        response.rateNQT = propertiesResponse.properties[0].value;
                        response.units = KRS.convertToQNT(String($(units_field).val()), decimals);
                        response.amountNQT = KRS.calculateOrderTotalNQT(response.units, response.rateNQT, decimals)
                    }
                }, { isAsync: false });
            } else {
        ...
        KRS.sendRequest("getCurrencyTransfers+", {
            ...
            if (response.transfers && response.transfers.length) {
                ...
                for (var i = 0; i < transfers.length; i++) {
                    ...
                    rows += "<tr>" +
                    "<td>" + KRS.getTransactionLink(transfers[i].transfer) + "</td>" +
    ...
    KRS.incoming.ignis = function () {
        KRS.pages.ignis();
    };


    KRS.setup.ignis = function() {
        KRS.sendRequest("getCurrency", { code: KRS.constants.IGNIS_CURRENCY_CODE }, function(response) {
            ...
            $('#ignis_shape_shift_select_coin').append('<option value="BTC_KPL">Bitcoin [BTC]</option>');
            $("#ignis_shape_shift_button").data("pair", "BTC_KPL");
    ...
    KRS.pages.ignis = function() {
        ...
        KRS.sendRequest("getCurrency", { code: KRS.constants.IGNIS_CURRENCY_CODE }, function(response) {
            ...
            KRS.sendRequest("getAccountCurrencies", {
                ...
                var balance = response.unconfirmedUnits ? KRS.formatQuantity(response.unconfirmedUnits, response.decimals) : "0";
                $("#your_ingis_balance_message").html(
                    $.t("ignis_message_12", { balance: balance })
                );
            });
            KRS.loadCurrencyOffers("sell", response.currency, false, true);
            KRS.getExchangeHistory(response.currency, false, "ignis_table");
        });
    };
    ...
    $("#ignis_shape_shift_load_button").on("click", function(e) {
        ...
        KRS.shapeShiftSelectCoins(inputFields, selectedCoins);
        $("#ignis_shape_shift_button").data("pair", "BTC_KPL");
    });
    ...
    $("#accept_ignis_tc_link").on("click", function(e) {
        e.preventDefault();
        KRS.updateSettings("ignis_tc_accepted", "1");
        KRS.pages.ignis();
    });
    ...
    $('#ignis_shape_shift_select_coin').change(function() {
        ...
        shapeShiftButton.data("pair", ignisSelectCoin.val() + "_KPL");
    });
