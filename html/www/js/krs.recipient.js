			...
			if (response.publicKey) {
				if (response.name){
					result = {
						...
						"message": $.t("recipient_info_with_name", {
							"name" : KRS.unescapeRespStr(response.name),
				...
				if (response.errorCode) {
					if (response.errorCode == 4) {
						...
						result = {
							"type": "danger",
							"message": $.t("recipient_problem") + " " + KRS.unescapeRespStr(response.errorDescription),
							"account": null
						};
					}
				} else {
			...
			if (address.set(account)) {
				...
				if (address.guess.length == 1) {
					callout.removeClass(classes).addClass("callout-danger").html($.t("recipient_malformed_suggestion", {
						"recipient": "<span class='malformed_address' data-address='" + KRS.escapeRespStr(address.guess[0]) + "' onclick='KRS.correctAddressMistake(this);'>" + address.format_guess(address.guess[0], account) + "</span>"
					})).show();
					...
					for (var i = 0; i < address.guess.length; i++) {
						html += "<li><span class='malformed_address' data-address='" + KRS.escapeRespStr(address.guess[i]) + "' onclick='KRS.correctAddressMistake(this);'>" + address.format_guess(address.guess[i], account) + "</span></li>";
					}
