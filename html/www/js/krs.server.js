...
var KRS = (function (KRS, $, undefined) {
    ...
    KRS.sendRequest = function (requestType, data, callback, options) {
        if (!options) {
            options = {};
        }
        ...
        if ("secretPhrase" in data) {
            ...
            if (accountId != KRS.account && !data.calculateFee) {
                ...
                KRS.processAjaxRequest(requestType, data, callback, options);
            }
        } else {
            KRS.processAjaxRequest(requestType, data, callback, options);
        }
    ...
    function isVolatileRequest(doNotSign, type, requestType, secretPhrase) {
        if (secretPhrase && KRS.isMobileApp()) {
            return true;
        }
        return (KRS.isPassphraseAtRisk() || doNotSign) && type == "POST" && !KRS.isSubmitPassphrase(requestType);
    }

    KRS.processAjaxRequest = function (requestType, data, callback, options) {
        var extra = null;
        if (data["_extra"]) {
            extra = data["_extra"];
            delete data["_extra"];
        ...
        var httpMethod = (KRS.isRequirePost(requestType) || "secretPhrase" in data || "doNotSign" in data || "adminPassword" in data ? "POST" : "GET");
        if (httpMethod == "GET") {
        ...
        if ((KRS.isRequirePost(requestType) || "secretPhrase" in data) &&
            KRS.isRequireBlockchain(requestType) && KRS.accountInfo.errorCode && KRS.accountInfo.errorCode == 5) {
            callback({
                "errorCode": 2,
        ...
        if (KRS.isScheduleRequest(requestType)) {
            ...
            if (isVolatile || data.calculateFee) {
                ...
                var keywordLength = KRS.constants.SCHEDULE_PREFIX.length;
        ...
        } else if (httpMethod == "POST" && KRS.rememberPassword) {
        ...
        if (options.remoteNode) {
            url = options.remoteNode.getUrl() + "/kpl";
        } else {
            url = KRS.getRequestPath(options.noProxy);
        }
        url += "?requestType=" + requestType;
        KRS.logConsole("Send request " + requestType + " to url " + url);
        ...
        $.ajax({
            ...
            if (!options.remoteNode && KRS.isConfirmResponse() &&
                !(response.errorCode || response.errorDescription || response.errorMessage || response.error)) {
                var requestRemoteNode = KRS.isMobileApp() ? KRS.getRemoteNode() : {address: "localhost", announcedAddress: "localhost"}; //TODO unify getRemoteNode with apiProxyPeer
                KRS.confirmResponse(requestType, data, response, requestRemoteNode);
            }
            ...
            if (secretPhrase && response.unsignedTransactionBytes && !data.doNotSign && !response.errorCode && !response.error && !data.calculateFee)  {
                ...
                if (file) {
                    ...
                    r.onload = function (e) {
                        ...
                        KRS.verifyAndSignTransactionBytes(response.unsignedTransactionBytes, signature, requestType, data, callback, response, extra, isVolatile);
                    };
                ...
                if (response.errorCode || response.errorDescription || response.errorMessage || response.error) {
                    ...
                    if (response.broadcasted == false && !data.calculateFee && !KRS.isScheduleRequest(requestType)) {
                        async.waterfall([
                            function (callback) {
                                addMissingData(data);
            ...
            KRS.logConsole("Node " + (options.remoteNode ? options.remoteNode.getUrl() : KRS.getRemoteNodeUrl()) + " received an error for request type " + requestType +
    ...
    KRS.verifyAndSignTransactionBytes = function (transactionBytes, signature, requestType, data, callback, response, extra, isVerifyECBlock) {
        ...
        if (!KRS.verifyTransactionBytes(byteArray, requestType, data, response.transactionJSON.attachment, isVerifyECBlock)) {
            callback({
                "errorCode": 1,
                "errorDescription": $.t("error_bytes_validation_server")
            }, data);
        ...
        if (data.broadcast == "false" && !isSchedule) {
            ...
            KRS.broadcastTransactionBytes(payload, callback, response, data, isSchedule, requestType);
        }
    };
    ...
    KRS.broadcastTransactionBytes = function (transactionData, callback, originalResponse, originalData, isSchedule, requestType) {
        var data = {
        ...
        if (isSchedule) {
            requestType = KRS.constants.SCHEDULE_PREFIX + requestType.substring(0, 1).toUpperCase() + requestType.substring(1);
            data.offerIssuer = originalData.offerIssuer; // Specific to currency buy
        } else {
            requestType = KRS.state.apiProxy ? "sendTransaction": "broadcastTransaction";
        }
        $.ajax({
            ...
            KRS.logConsole("request failed, status: " + textStatus + ", error: " + error);
            if (KRS.console) {
                KRS.addToConsole(this.url, this.type, this.data, error, true);
            }
            KRS.resetRemoteNode(true);
    ...
    KRS.generateQRCode = function(target, qrCodeData, minType, cellSize) {
        ...
        while (type <= 40) {
            try {
                ...
                KRS.logConsole("Encoded QR code of type " + type);
                if (target) {
