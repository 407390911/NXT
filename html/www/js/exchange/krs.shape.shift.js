...
var KRS = (function(KRS, $) {
    ...
    var coinToPair = function (op, coin) {
        return (op == "buy") ? "KPL_" + coin : coin + "_KPL";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("KPL_") == 0) {
            return pair.substring("KPL_".length);
        }
    ...
    var addDepositAddress = function(address, pair) {
        var pairParts = pair.split('_');
        KRS.addDepositAddress(address, pairParts[0], pairParts[1], DEPOSIT_ADDRESSES_KEY + KRS.accountRS);
    };
    ...
    var apiCall = function(action, requestData, method, doneCallback, ignoreError, modal) {
        ...
        $.ajax({
            url: KRS.getShapeShiftUrl() + action,
            crossDomain: true,
            dataType: "json",
            type: method,
            timeout: 30000,
                    ...
                    async.waterfall([
                        ...
                        if (op == "sell") {
                            if (parseFloat(data.rate) == 0) {
                                ...
                                rate = KRS.invert(data.rate);
                            }
                            if (parseFloat(data.quotedRate) == 0) {
                                quotedRate = "N/A";
                                diff = "N/A";
                            } else {
                                quotedRate = KRS.invert(data.quotedRate);
            ...
            tasks.push((function (i) {
                return function (callback) {
                    ...
                    apiCall("txStat/" + depositAddresses[i].address, {}, "GET", function(data) {
                        ...
                        row += "<td>" + KRS.getExchangeAddressLink(data.address, depositAddresses[i].from) + "</td>";
                        ...
                        row += "<td>" + KRS.getExchangeAddressLink(data.withdraw, depositAddresses[i].to) + "</td>";
                        row += "<td>" + data.outgoingCoin + "</td>";
                        row += "<td>" + data.outgoingType + "</td>";
                        row += "<td>" + KRS.getExchangeTransactionLink(data.transaction, depositAddresses[i].to) + "</td>";
                        KRS.logConsole(row);
                        callback(null, row);
                    }, true);
                }
    ...
    KRS.shapeShiftSelectCoins = function(inputFields, selectedCoins) {
    ...
    KRS.pages.exchange_shape_shift = function() {
    ...
    refreshPage = function() {
        if (KRS.currentPage == "exchange_shape_shift") {
            KRS.pages.exchange_shape_shift();
        }
    };
    ...
    $("#shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#shape_shift_buy_title").html($.t("exchange_kpl_to_coin_shift", { coin: coin }));
        $("#shape_shift_buy_min").val(invoker.data("min"));
        $("#shape_shift_buy_min_coin").html("KPL");
        $("#shape_shift_buy_max").val(invoker.data("max"));
        $("#shape_shift_buy_max_coin").html("KPL");
        $("#shape_shift_buy_rate").val(invoker.data("rate"));
        $("#shape_shift_buy_rate_text").html("KPL/" + coin);
    ...
    $("#shape_shift_buy_submit").on("click", function(e) {
        ...
        var amountNQT = KRS.convertToNQT($("#shape_shift_buy_amount").val());
        ...
        apiCall('shift', {
            ...
            apiKey: KRS.settings.shape_shift_api_key
        }, 'POST', function (data) {
    ...
    $("#shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        KRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#shape_shift_sell_title").html($.t("exchange_coin_to_kpl_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#shape_shift_sell_rate_text").html(coin + "/KPL");
                $("#shape_shift_sell_fee").val(data.fee);
                $("#shape_shift_sell_fee_coin").html("KPL");
                ...
                apiCall('shift', {
                    ...
                    KRS.generateQRCode("#shape_shift_sell_qr_code", data.deposit);
                    callback(null);
                })
            }
        ], function (err, result) {
    ...
    $('#m_send_amount_sell_withdrawal_amount').change(function () {
        ...
        $("#m_send_amount_sell_qr_code").html("<span style='color: blue'>" + $.t("please_enter_withdrawal_amount") + "</span>");
        modal.css('cursor','wait');
        apiCall('sendamount', { amount: amount, withdrawal: KRS.accountRS, pubKey: publicKey, pair: pair, apiKey: KRS.settings.shape_shift_api_key },
                "POST", function (data) {
            try {
                ...
                modal.find(".error_message").html("").hide();
                rate.val(KRS.invert(data.success.quotedRate));
                fee.val(data.success.minerFee);
                ...
                KRS.generateQRCode("#m_send_amount_sell_qr_code", "bitcoin:" + data.success.deposit + "?amount=" + data.success.depositAmount);
