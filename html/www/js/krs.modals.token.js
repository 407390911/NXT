...
var KRS = (function(KRS, $) {
    ...
    $("#generate_token_button").click(function (e) {
		var data = KRS.getFormData($("#generate_token_form"));
        ...
        if (!KRS.rememberPassword) {
			...
			var accountId = KRS.getAccountId(secretPhrase);
			if (accountId != KRS.account && !isOffline) {
        ...
        var token = KRS.generateToken(website, secretPhrase);
        ...
        KRS.generateQRCode("#generate_token_output_qr_code", token, 14);
        outputQrCodeContainer.show();
        e.preventDefault();
    });

    KRS.setTokenPassword = function(password) {
        _password = password;
    };
