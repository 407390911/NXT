...
loader.load(function(KRS) {
    var data = {
        recipient: KRS.getAccountIdFromPublicKey(config.recipientPublicKey), // public key to account id
        recipientPublicKey: config.recipientPublicKey, // Optional - public key announcement to init a new account
        amountNQT: KRS.convertToNQT("1.234"), // KPL to NQT conversion
    ...
    data = Object.assign(
        ...
        KRS.encryptMessage(KRS, "note to myself", config.secretPhrase, KRS.getPublicKey(converters.stringToHexString(config.secretPhrase)), true),
    ...
    KRS.sendRequest("sendMoney", data, function (response) {
