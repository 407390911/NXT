...
var KRS = (function(KRS, $) {
	...
	KRS.getMarketplaceItemHTML = function(good) {
		var html = "";
		var id = 'good_'+ KRS.escapeRespStr(good.goods);
				...
				html += "<tr>";
					...
					html += "<td>";
						html += "<div style='float:right;color: #999999;background:white;padding:5px;border:1px solid #ccc;border-radius:3px'>";
							...
							html += "<strong>" + $.t("product_id") + "</strong>: &nbsp;<a href='#'' data-toggle='modal' data-target='#dgs_product_modal' data-goods='" + KRS.escapeRespStr(good.goods) + "'>" + KRS.escapeRespStr(good.goods) + "</a><br>";
							html += "<strong>" + $.t("timestamp_listing") + "</strong>: " + KRS.formatTimestamp(good.timestamp);
        				html += "</div>";
						html += "<div><h3 class='title'><a href='#' data-goods='" + KRS.escapeRespStr(good.goods) + "' data-toggle='modal' data-target='#dgs_purchase_modal'>" + KRS.escapeRespStr(good.name) + "</a></h3></div>";
						...
						html += "<div class='showmore'><div class='moreblock description'>" + String(good.description).autoLink().nl2br() + "</div>";
							if (good.numberOfPublicFeedbacks > 0) {
								html += "<span style='float:right;clear:right;'><a href='#' class='feedback' data-goods='" + KRS.escapeRespStr(good.goods) + "' data-toggle='modal' data-target='#dgs_show_feedback_modal'>" + $.t('show_feedback', 'Show Feedback') + "</a></span>";
							}
						html += "</div>";
						html += "<div>";
							...
							for (var i=0; i<tags.length; i++) {
								...
								html += '<a href="#" class="tags" onclick="event.preventDefault(); KRS.dgs_search_tag(\'' + KRS.escapeRespStr(tags[i]) + '\');">';
								html += KRS.escapeRespStr(tags[i]) + '</a>';
								html += '</span>';
							}
	...
	KRS.getMarketplacePurchaseHTML = function(purchase, showBuyer) {
		...
		return "<div data-purchase='" + KRS.escapeRespStr(purchase.purchase) + "' " + (purchase.unconfirmed ? "class='tentative'" : "") + "><div style='float:right;color: #999999;background:white;padding:5px;border:1px solid #ccc;border-radius:3px'>" +
			...
			"<strong>" + $.t("product_id") + "</strong>: &nbsp;<a href='#'' data-toggle='modal' data-target='#dgs_product_modal' data-goods='" + KRS.escapeRespStr(purchase.goods) + "'>" + KRS.escapeRespStr(purchase.goods) + "</a>" +
			"</div>" +
			"<h3 class='title'><a href='#' data-purchase='" + KRS.escapeRespStr(purchase.purchase) + "' data-toggle='modal' data-target='" + (modal ? modal : "#dgs_view_delivery_modal") + "'>" + KRS.escapeRespStr(purchase.name) + "</a></h3>" +
			"<table>" +
	...
	KRS.getMarketplacePendingOrderHTML = function(purchase) {
      return "<div data-purchase='" + KRS.escapeRespStr(purchase.purchase) + "'><div style='float:right;color: #999999;background:white;padding:5px;border:1px solid #ccc;border-radius:3px'>" +
			"<strong>" + $.t("buyer") + "</strong>: <span>" + KRS.getAccountLink(purchase, "buyer") + "</span><br>" +
			"<strong>" + $.t("product_id") + "</strong>: &nbsp;<a href='#'' data-toggle='modal' data-target='#dgs_product_modal' data-goods='" + KRS.escapeRespStr(purchase.goods) + "'>" + KRS.escapeRespStr(purchase.goods) + "</a>" +
			"</div>" +
			"<h3 class='title'><a href='#' data-purchase='" + KRS.escapeRespStr(purchase.purchase) + "' data-toggle='modal' data-target='#dgs_view_purchase_modal'>" + KRS.escapeRespStr(purchase.name) + "</a></h3>" +
			...
			"<tr><td><strong>" + $.t("quantity") + "</strong>:</td><td>" + KRS.format(purchase.quantity) + "</td></tr>" +
			"</table>" +
			"<span class='delivery'><button type='button' class='btn btn-default btn-deliver' data-toggle='modal' data-target='#dgs_delivery_modal' data-purchase='" + KRS.escapeRespStr(purchase.purchase) + "'>" + $.t("deliver_goods") + "</button></span>" +
			"</div><hr />";
	};
		...
		KRS.sendRequest("getDGSTags+", {
				...
				if (response.tags && response.tags.length) {
					...
					for (var i=0; i<response.tags.length; i++) {
						...
						content += KRS.escapeRespStr(response.tags[i].tag) + ' [' + response.tags[i].inStockCount + ']</a>';
						content += '</div>';
					}
				}
         ...
         var rows = "";
			if (response.goods && response.goods.length) {
				...
				for (var i = 0; i < response.goods.length; i++) {
               		...
               		rows += "<tr class='' data-goods='" + KRS.escapeRespStr(good.goods) + "'><td><a href='#' data-toggle='modal' data-target='#dgs_product_modal' data-goods='" + KRS.escapeRespStr(good.goods) + "'>" + KRS.escapeRespStr(good.name) + "</a></td>";
					...
					rows += "<td style='white-space:nowrap'><a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#dgs_price_change_modal' data-goods='" + KRS.escapeRespStr(good.goods) + "'>" + $.t("change_price") + "</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#dgs_quantity_change_modal' data-goods='" + KRS.escapeRespStr(good.goods) + "'>" + $.t("change_qty") + "</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#dgs_delisting_modal' data-goods='" + KRS.escapeRespStr(good.goods) + "'>" + $.t("delete") + "</a></td></tr>";
				}
			}
			KRS.dataLoaded(rows);
		});
	...
	KRS.forms.dgsListingComplete = function(response, data) {
		...
		if (KRS.currentPage == "my_dgs_listings") {
         var rowToAdd = "<tr class='tentative' data-goods='" + KRS.escapeRespStr(response.transaction) + "'><td><a href='#' data-toggle='modal' data-target='#dgs_listing_modal' data-goods='" + KRS.escapeRespStr(response.transaction) + "'>" + KRS.escapeRespStr(data.name) + "</a></td><td class='quantity'>" + KRS.format(data.quantity) + "</td><td class='price'>" + KRS.formatAmount(data.priceNQT) + " KPL</td><td style='white-space:nowrap'><a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#dgs_price_change_modal' data-goods='" + KRS.escapeRespStr(response.transaction) + "'>" + $.t("change_price") + "</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#dgs_quantity_change_modal' data-goods='" + KRS.escapeRespStr(response.transaction) + "'>" + $.t("change_qty") + "</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#dgs_delisting_modal' data-goods='" + KRS.escapeRespStr(response.transaction) + "'>" + $.t("delete") + "</a></td></tr>";
	...
	KRS.forms.dgsPurchase = function($modal) {
        ...
        if (KRS.isShowFakeWarning()) {
            return KRS.composeFakeWarning($.t("goods"), data.goods);
        }
			...
			if (response.errorCode) {
				...
				KRS.sendRequest("getDGSGood", {
					...
					if (good.errorCode) {
						...
						var output = "<table>";
						var image = KRS.dgs_get_picture(good);
						output += "<tr><th style='width:85px;'><strong>" + $.t("product") + "</strong>:</th><td>" + KRS.escapeRespStr(good.name) + '</td><td rowspan = 20 height="100" width="100">' + image + '</td></tr>';
		...
		KRS.sendRequest("getDGSGood", {
			...
			if (response.errorCode) {
				...
				KRS.sendRequest("getDGSGoodsPurchases+", {
					...
					output += "<tr><th style='width:85px'><strong>" + $.t("product") + "</strong>:</th><td>" + KRS.escapeRespStr(response.name);
					if (publicFeedback.purchases.length && publicFeedback.purchases.length > 0) {
						output += " (<a href='#' class='feedback' data-goods='" + KRS.escapeRespStr(response.goods) + "' data-toggle='modal' data-target='#dgs_show_feedback_modal'>" + $.t('show_feedback', 'Show Feedback') + "</a>)";
					}
					...
					if (type == "dgs_purchase_modal") {
						KRS.modalStack.push({class: "dgs_show_modal_action_purchase", key: "data-goods", value: goods});
					} else if (type == "dgs_product_modal") {
						KRS.modalStack.push({class: "dgs_show_modal_action_product", key: "data-goods", value: goods});
					}
					...
					if (type == "dgs_quantity_change_modal") {
						$("#dgs_quantity_change_current_quantity, #dgs_quantity_change_quantity").val(KRS.escapeRespStr(response.quantity));
						...
						$("#dgs_purchase_price").val(KRS.escapeRespStr(response.priceNQT));
			...
			if (response.purchases.length && response.purchases.length > 0) {
				for (var i=0; i<response.purchases.length; i++) {
					...
					if (purchase.publicFeedbacks.length && purchase.publicFeedbacks.length > 0) {
						$modal.find(".modal_content table").append('<tr><td>' + KRS.escapeRespStr(purchase.publicFeedbacks[0]) + '</td></tr>');
					}
				}
			}
		});
    ...
    dgsShowPictureModal.on("show.bs.modal", function(e) {
		...
		KRS.sendRequest("getDGSGood+", {
			...
			if (!response.hasImage) {
				...
				image = KRS.getRequestPath() + "?requestType=downloadPrunableMessage&transaction=" + response.goods + "&retrieve=true";
			}
			$("#dgs_product_picture_modal").attr("src", image);
			$("#dgs_product_picture_modal_goods_name").html(KRS.escapeRespStr(response.name));
		});
    });
    ...
    KRS.dgs_get_picture = function(input) {
        ...
        if (!input.hasImage) {
            ...
            picture.src = KRS.getRequestPath() + "?requestType=downloadPrunableMessage&transaction=" + input.goods + "&retrieve=true";
	...
	KRS.dgs_listings = function (table, api, full) {
    		...
    		KRS.sendRequest(api, params, function (response) {
    			...
    			for (var i = 0; i < response.length; i++) {
    				var item = response[i];
    				var name = KRS.escapeRespStr(item.name);
