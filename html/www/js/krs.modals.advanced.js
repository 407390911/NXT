...
var KRS = (function(KRS, $, undefined) {
	...
	KRS.setAdvancedModalPassword = function (password) {
		_password = password;
	};
	...
	KRS.showRawTransactionModal = function(transaction) {
        if (transaction.unsignedTransactionBytes && !transaction.transactionBytes) {
            ...
            KRS.generateQRCode("#raw_transaction_modal_unsigned_bytes_qr_code", transaction.unsignedTransactionBytes, 14);
        ...
        if (transaction.transactionJSON) {
            ...
            if (window.URL) {
                ...
                try {
                    ...
                    KRS.logConsole("Desktop Application in Java 8 does not support createObjectURL");
                    downloadLink.hide();
				}
            } else {
                downloadLink.hide();
    ...
    $(".qr_code_reader_link").click(function(e) {
        ...
        KRS.scanQRCode(readerId, function(data) {
            $("#" + outputId).val(data);
        });
    });
	...
	function updateSignature(signature) {
		...
		KRS.generateQRCode("#transaction_signature_qr_code", signature, 8);
    ...
    KRS.forms.signTransaction = function() {
		...
		if (data.unsignedTransactionBytes && !data.validate) {
			KRS.logConsole("Sign transaction locally");
			var output = {};
			var secretPhrase = (KRS.rememberPassword ? _password : data.secretPhrase);
			var isOffline = $(".mobile-offline").val();
			if (KRS.getAccountId(secretPhrase) == KRS.account || isOffline) {
				try {
					var signature = KRS.signBytes(data.unsignedTransactionBytes, converters.stringToHexString(secretPhrase));
