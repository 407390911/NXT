...
var KRS = (function(KRS, $, undefined) {
	...
	KRS.updateNotificationUI = function() {
		...
		if (totalCount > 0) {
			...
			$menuItem.find('.nm_inner_subtype').css('backgroundColor', '#d1a400');
	...
	KRS.setUnconfirmedNotifications = function() {
        var count;
        if (KRS.unconfirmedTransactions.length > KRS.itemsPerPage) {
            count = String(KRS.itemsPerPage) + "+";
        } else {
            count = String(KRS.unconfirmedTransactions.length);
        }
	...
	KRS.setShufflingNotifications = function() {
        KRS.sendRequest("getAllShufflings", {},
            function (response) {
