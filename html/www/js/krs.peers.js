        ...
        KRS.sendRequest("getPeers+", {
			...
			if (response.peers && response.peers.length) {
				...
				for (var i = 0; i < response.peers.length; i++) {
					...
					rows += "&nbsp;&nbsp;" + (peer.announcedAddress ? KRS.getPeerLink(peer.announcedAddress) : $.t("unknown")) + "</td>";
					...
					rows += (peer.application && peer.version ? KRS.escapeRespStr(peer.application) + " " + KRS.escapeRespStr(peer.version) : "?") + "</label></td>";
